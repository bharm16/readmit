...
| Cancellation while admission waits | `TestACancellationDuringExecutionAdmissionIsCancelledNotDenied`: with a retained clock update, every operation whose declared profile takes an execution, every other interruptible one that admits the author, and a local write, each cancelled during admission, answer cancelled without waiting for admission to give up, never permission denied | Admission contention simulated by the retained update file |
