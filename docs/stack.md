...
- Local shell state is eight bounded, versioned documents: recently opened folders (`readmit-desktop-recent/v1`), saved filters and the active selection (`readmit-filters/v1`), the unstored working session (`readmit-desktop-session/v1`), editor drafts (`readmit-desktop-drafts/v1`, or `/v2` while a draft names the catalog object it edits), the projects folder and the projects this viewer opened (`readmit-desktop-projects/v1`), and the paths the person selected for the operation policy (`readmit-desktop-operation-selection/v1`), commercial destinations (`readmit-desktop-commercial-selection/v1`) and customer hub configuration (`readmit-desktop-hub-selection/v1`). The session retains the view and unstored notes; the draft store holds editors' unstored work under internal identities. Saved filters and drafts can contain patient data the operator typed, so all eight are owner-readable and kept outside evidence. An unreadable selection is reported until the person chooses a replacement; restoring a session never resumes or resends network work. No telemetry, crash reporting, update checks, or evidence in browser storage.
