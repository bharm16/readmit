...
Three Go modules. `github.com/bharm16/readmit` holds the engine and produces the released `readmit` executable. `github.com/bharm16/readmit/desktop` holds the desktop shell and requires the first through a `replace` directive, so the webview dependency graph never enters the released module. `github.com/bharm16/readmit/hub` is the separate customer-operated Go service, with PostgreSQL metadata and private local artifact storage. The desktop shell imports its strict readers and two offline checks for administration handoffs; this includes the hub module's dependency graph in the desktop build, while the released static CLI remains separate. The hub supports an explicit team mode with pinned OIDC-provider access-token validation, project roles and certificate-bound scoped tokens; operator-only mode remains separate. See the [hub installation guide](../hub/README.md). Ordinary engine internal packages with explicit constructor parameters. No dependency-injection container or plugin loader in the engine: commands and the desktop facade call the same small domain packages directly.
...
- `internal/desktop` is the typed Go facade for evidence work. The desktop module also binds the narrow `desktop/hubadmin.Admin` reader for local customer-hub host handoffs, under [ADR-0005](adr/0005-desktop-shell-is-a-separate-module-over-a-typed-go-facade.md#customer-hub-host-administration-amended-2026-09-24). Desktop operations return typed results with one explicit state each. The interface never parses command output and never reimplements HL7 or case bundle semantics. See [the desktop contract](desktop.md).
- The interface's TypeScript declarations of both bound objects are generated from the Go types by `desktop/bindgen`, a Go program in the desktop module built from the standard library's reflection and parser, rather than by Wails' own generator or an npm package; its test fails while the committed declarations drift. See [typed bindings](desktop.md#typed-bindings).
...
- Local shell state is seven bounded, versioned documents: saved filters and the active selection (`readmit-filters/v1`), the working session (`readmit-desktop-session/v1`), editor drafts (`readmit-desktop-drafts/v1`, or `/v2` while a draft names the catalog object it edits), the projects folder and the projects this viewer opened (`readmit-desktop-projects/v1`), and the paths the person selected for the operation policy (`readmit-desktop-operation-selection/v1`), commercial destinations (`readmit-desktop-commercial-selection/v1`) and customer hub configuration (`readmit-desktop-hub-selection/v1`). The session retains the view; the draft store holds every editor's unstored work, notes included, under internal identities, and recovery restores editors from it, never a send, reset or approval. Saved filters and drafts can contain patient data the operator typed, so all seven are owner-readable and kept outside evidence. An unreadable selection is reported until the person chooses a replacement; restoring a session never resumes or resends network work. No telemetry, crash reporting, update checks, or evidence in browser storage.
...
- `internal/destination` opens every connection readmit makes to a configured destination: a send, an observation's read, the one connection a fixture reset may open, and a connectivity check. It asks `internal/sendpolicy` the question that purpose asks, retains the decision through the caller's recorder before using it, admits the connection or refuses it, dials only the one address the decision checked — never a second resolution of the configured name — and completes TLS through `internal/transportsecurity`. The name's resolver is its seam. A failure is named once, in one classification a send maps onto its `readmit-run/v1` classes and a diagnosis onto its outcomes. A check reports the decision and reaches the configured address whatever it says.
- A fixture reset is `readmit target reset` over a `readmit-reset-plan/v1` document the operator selects explicitly. `internal/fixturereset` owns a closed, reviewed set of typed Go reset operators, each declaring the one authority it requires; the contract carries no command, script, interpreter or argument, and a test spec names no reset action. A reset runs only against a recorded nonproduction environment, asks the same send decision for the one connection it may open and opens it only to the address that decision checked, and retains a `readmit-reset-outcome/v1` document whose states are the execution vocabulary `internal/durablerun` owns. See [named test environments](target.md).
