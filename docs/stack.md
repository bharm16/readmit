...
- Each Go job owns a compiler/platform/dependency-scoped cache that only runs on main advance with the commit; every other run restores main's newest one and saves nothing. Desktop cache identity includes both module checksum files. Superseded PR runs are cancelled; main, release-tag, daily and dispatched runs are independent.
