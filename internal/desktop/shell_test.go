...
func TestEveryStatusIsDistinguishableWithoutColour(t *testing.T) {
	...
	for _, kind := range []desktop.Kind{
		...
		desktop.PacketArtifact, desktop.PortableReviewArtifact,
...
func TestPrivacyStatusNamesWhatIsAbsentAndWhatIsKept(t *testing.T) {
	...
	for _, document := range []string{"readmit-desktop-recent/v1", "readmit-filters/v1", desktop.SessionSchema, "readmit-correlation-review/v1"} {
