...
func TestCustomerCIExamplesExecuteSavedSuiteAndPropagateFailures(t *testing.T) {
	...
	for example, command := range commands {
		t.Run(fmt.Sprintf("example-%d", example), func(t *testing.T) {
			exerciseCustomerCI(t, func(t *testing.T, root string) *exec.Cmd {
...
// Direct argv execution covers the public CI contract on every native platform.
// POSIX documentation examples are independently exercised above.
func TestCustomerCIPublicCommandPropagatesFailures(t *testing.T) {
	exerciseCustomerCI(t, func(t *testing.T, root string) *exec.Cmd {
		return exec.Command(binary, "--operation-policy", testlicense.New(t), "suite", "ci",
			filepath.Join(root, "suite.json"), "--environment", "lab", "--output",
			filepath.Join(root, "run"), "--requirements", filepath.Join(root, "coverage.json"),
			"--send", "--deadline", "5m")
	})
}

func exerciseCustomerCI(t *testing.T, command func(*testing.T, string) *exec.Cmd) {
	t.Helper()
	for _, ackCode := range []string{"AA", "AE"} {
		t.Run(ackCode, func(t *testing.T) {
			...
			if cmd.ProcessState == nil {
				t.Fatalf("CI process did not start: %v", err)
			}
