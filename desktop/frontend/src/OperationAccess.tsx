...
import {
  ...
  renewLicenseDocument, resolveOperationClock, verifyLicenseDocument,
 ...
 * The pane is the License page: it shows this device's license with one
 * primary activation or renewal action, keeps the administrator's
...
export function OperationAccess() {
  ...
  return <section aria-labelledby="operation-access-title">
    <h3 id="operation-access-title">License</h3>
    ...
    />
    {/* The supplied-folder workflow is administrator work, not a second
     * activation task beside the device's own: it keeps its distinct status,
     * renewal, release and export operations inside one named subview. */}
    <ControlledDetails summary="Administrator setup" className="administrator-setup">
      <p>An administrator can supply a prepared activation folder for this window, or build one from a received license and its trust document below.</p>
      ...
      <div className="operation-actions">
        ...
        <button disabled={busy || !result?.selected} onClick={() => void perform(activateOperations, showOperationResult)}>Activate</button>
        <IconButton label="Refresh activation" icon="refresh" disabled={busy} onClick={() => void perform(operationStatus, setResult)} />
        {/* Clock resolution belongs to the clock problem it resolves; the
         * status above explains when there is nothing to resolve. */}
        {result?.clock?.rollback ? (
          <button disabled={busy} onClick={() => void perform(resolveOperationClock, showOperationResult)}>Resolve clock change</button>
        ) : null}
        <button disabled={busy || !result?.clock || result.clock.released} onClick={() => void perform(releaseOperations, showOperationResult)}>Release activation</button>
        ...
        })}>Renew activation…</button>
        <button disabled={busy} onClick={() => void perform(exportLicenseDocument, setExported)}>Export license…</button>
        <button disabled={busy} onClick={() => void perform(async () => {
          const choice = await chooseOperationPolicy();
          return choice.state === "completed" ? operationStatus() : choice;
        }, showOperationResult)}>Choose activation folder…</button>
      </div>
      ...
      {verified ? (
        <div className="license-verified">
          ...
          {document?.operation_capable ? <>
            <label htmlFor="license-author">Author</label>
            ...
            })}>Choose activation folder…</button>
            ...
            })}>Create activation folder</button>
    ...
    <div className="commercial-access">
      ...
      {commercial?.state === "completed" && commercial.portal ? <>
        ...
        <p><a href={commercial.portal} target="_blank" rel="noreferrer">Manage account</a></p>
      </> : <p role="note">{commercial?.reason ?? "the account portal destination is not configured; an administrator can choose the operator-supplied destinations file in Administrator setup"}</p>}
      <ul>
