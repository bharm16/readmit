...
test("the hub administration handoff reads a real configuration and never runs its host step", async () => {
  ...
  await enter(user, admin.getByLabelText("Local configuration copy"), config);
  await enter(user, admin.getByLabelText("Configuration path on hub host"), "/etc/readmit-hub/config.json");
  ...
  await enter(user, admin.getByLabelText("Configuration path on hub host"), "relative.json");
  ...
  await press(user, admin.getByRole("button", { name: "Clear preview" }));
  expect(await admin.findByText("Preview cleared; no host action was taken.")).toBeTruthy();
