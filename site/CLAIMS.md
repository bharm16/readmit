...
| Policy-driven redaction with a fail-closed export review | [docs/redact.md](../docs/redact.md) | `TestDeriveLocatesEveryUnreviewedSurface` |
...
| The published candidate is `v0.1.0-alpha.2`, containing `readmit inspect` only; CI's archives and desktop packages are previews of an unpublished release, and a published complete product does not exist yet | [docs/support-matrix.md](../docs/support-matrix.md), [docs/release-acceptance.md](../docs/release-acceptance.md), [.github/workflows/ci.yml](../.github/workflows/ci.yml) | status |
| Desktop packages (`.deb`, `.dmg`, `.pkg`, `.msi`) are built natively on five targets, installation-tested, never published, and unsigned for distribution; each build writes a `readmit-desktop-package/v1` manifest recording `signed_for_distribution: false` | [docs/desktop.md](../docs/desktop.md), [desktop/packaging/packages.json](../desktop/packaging/packages.json), [.github/workflows/desktop.yml](../.github/workflows/desktop.yml) | `TestCommandLineReleaseNeverReachesTheDesktopShell`, `TestDesktopPackagesDeclareThePrerequisitesTheDocumentationPromises` |
| Each package declares its platform prerequisites, and none downloads one: WebKitGTK and GTK for the `.deb`, the operating system's webview for macOS, the WebView2 Runtime as an MSI launch condition that names the offline standalone installer | [docs/desktop.md](../docs/desktop.md) | `TestDesktopPackagesDeclareThePrerequisitesTheDocumentationPromises` |
| A manifest claiming a distribution signature is refused; Apple silicon carries only the linker's ad-hoc signature, which names no authority | [docs/desktop.md](../docs/desktop.md), [tools/package_desktop.py](../tools/package_desktop.py) | status |
...
| Normalization and ignore policies, with every suppressed difference shown and attributed | [docs/normalize.md](../docs/normalize.md) | `TestNormalizeAcceptanceFixtureShowsEveryRuleAndEverySuppression`, `TestNormalizeCannotSuppressEvidenceNoRuleCanAddress`, `TestNormalizeNeverDisplaysAValueTheComparisonRead` |
...
| Downstream capture observation: one declared field selector, no receipt asked of the system under test, the capture never written to | [docs/observe.md](../docs/observe.md), [docs/support-matrix.md](../docs/support-matrix.md) | `TestObserveCollectBindsDownstreamMessageEvidenceToWhatTheRunProduced`, `TestACaptureObservationNeverReadsFailedCollectionAsAbsence`, `TestTheDeclaredScopeDecidesWhichOccurrencesAreRecords` (package tests in `internal/observesource/capture_test.go`) |
...
| Database, Mirth/OIE, SSH, scheduling, plain HTTP: not available | [docs/support-matrix.md](../docs/support-matrix.md), [docs/observe.md](../docs/observe.md), [docs/source.md](../docs/source.md), [docs/product-decisions.md](../docs/product-decisions.md) | status |
...
| Redact and export a synthetic case, including the blocked policy | [docs/redact.md](../docs/redact.md) | `TestRedactExecutableGeneratesOnlyDerivedProofAndNoPlantedValues`, `TestDeriveLocatesEveryUnreviewedSurface` |
| The desktop sample workspace holds the command line's generated bytes and adds two entries that are not evidence: a loopback practice endpoint and a states-only index of the sample case | [docs/desktop.md](../docs/desktop.md), [docs/guided-sample.md](../docs/guided-sample.md) | `TestDesktopSampleWorkspaceHoldsTheCommandLineFamilyBytes`, `TestPreparingTheWorkspaceMakesTheGeneratedFamilyWritable` |
| The guided sample is completed in the window alone, persisting the real case, spec and both runs, with progress read back from the folder | [docs/guided-sample.md](../docs/guided-sample.md) | `TestTheGuidedSampleIsCompletedThroughTheWindowAlone`, `TestEveryStepIsReadBackFromTheWorkspaceAndNothingIsRemembered` |
