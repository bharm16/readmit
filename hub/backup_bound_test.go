...
func TestBackupV4MaximumCataloguesAndLifecycleReadBack(t *testing.T) {
	team := true
	m := backupManifest{Schema: "readmit-hub-backup/v4", MetadataVersion: 5, Team: &team, Artifacts: make([]backupEntry, 65536), Projects: make([]projectLink, 65536), Reviews: []ReviewEvent{}, Lifecycle: make([]LifecycleEvent, maxLifecycle)}
	project := strings.Repeat("z", 64)
	...
	m.Reviews = make([]ReviewEvent, maxReviews)
	...
	if len(got.Reviews) != maxReviews || len(got.Lifecycle) != maxLifecycle || len(got.Artifacts) != 65536 || len(got.Projects) != 65536 {
